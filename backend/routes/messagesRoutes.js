...
router.get("/getMessages/:userId", async (req, res) => {
  ...
  try {
    ...
    let users = await User.find({ _id: { $in: Array.from(uniqueUserIds) } })
      // --- THIS IS THE FIX ---
      .select("_id name email profileImage") // Added profileImage here
      .lean();
...
router.get("/getunreadcount/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "something went wrong while getting  count" });
  }
});
...
router.post('/markAsRead', async (req, res) => {
    try {
        const { roomId, userId } // userId is the MongoDB _id of the current user
        = req.body;
        ...
        // Update messages where the current user is the receiver and seen is false
        await Message.updateMany(
            { roomid: roomId, receiver: userId, seen: false },
            { $set: { seen: true } }
        );
