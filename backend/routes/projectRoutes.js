...
router.post("/saveproposal", async (req, res) => {
  try {
    const { freelancer, project, coverLetter, totalBidAmount, deliveryTime } =
      req.body;
    ...
    if (!existingProject.client) {
      return res
        .status(400)
        .json({ message: "Project has no client assigned" });
    }
...
router.get("/proposals/:clientId", async (req, res) => {
  try {
    ...
    const projects = await Project.find({ client: clientUser._id }).select(
      "_id"
    );
...
router.get("/completed/all", async (req, res) => {
  try {
    ...
    const recentProjects = await Project.find({ status: "completed" })
      ...
      .populate("assignedFreelancer", "name rating profileImage");
...
router.put("/:projectId/statuspendingreview", async (req, res) => {
  try {
    ...
    if (status !== "pending-review") {
      return res
        .status(400)
        .json({ message: "Invalid status for this action." });
    }
...
// Add this new route to your projectRoutes.js file
...
router.get('/:projectId/proposals', async (req, res) => {
    try {
        ...
        console.error('Error fetching proposals for project:', error);
        res.status(500).json({ message: 'Server Error' });
    }
});
...
router.post('/:projectId/review', async (req, res) => {
    try {
        ...
        if (reviewee) {
            ...
            reviewee.rating = totalRating / reviews.length;
